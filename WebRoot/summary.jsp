...
<script>
	...
	function check_time(id_a, id_b) {
		var b_value = new Date(document.getElementById(id_a).value.replace(/-/g, '/')).getTime(), e_value = new Date(document.getElementById(id_b).value.replace(/-/g, '/')).getTime();
		if (e_value < b_value) {
			alert(lg.distime);
			return false;
		}
	...
	function viewChart() {
		var $table = $('#listtable'), $chartWrapper = $('#chart-wrapper'), xAxis = [], mileageSerie = {
			name : '里程(公里)',
			data : []
		}, outSpeedSerie = {
			name : '超速(次)',
		...
		$table.find('tr[data-user-name]').each(function(i, v) {
			var $this = $(this), userName = $this.data('user-name'), mileage = +$this.data('mileage'), outSpeed = +$this.data('out-speed'), stay = +$this.data('stay');
			xAxis.push(userName);
...
<body id="run_summary">
	...
	<div id="bd">
		...
		<div class="col-main" id="col-main" style="padding: 0!important;">
			<div class="mod-report">
				<div class="hd">
					<h2>
						运行统计总览 [<span id="tip-btn" style="color: #0077FF;cursor: pointer;" title="">?</span>]
					</h2>
				</div>
				<div class="bd">
					<div id="tip-content" class="description">
						...
						<dl>
							...
							<dd>
								答：因为两个页面采用的统计方式不一样。<br> 1 总览统计里 是系统每晚，根据当时设置的超速值 来统计并保存当天的超速数据。(即如果第二天修改超速值也不会影响前一天的超速次数)<br> 2 详单页面 是实时去取GPS数据来进行统计，根据最新的超速值来进行统计。<br> 因此，查询多天时，其间如果设备超速值有修改过，则两边的次数就可能不一样。<br>
							</dd>
						</dl>
					...
					<div class="block-forms">
						<div class="rr">
							<form id="submit-form" onSubmit="return onSubmit();" action="javascript://" method="get" autocomplete="off">
								<table border="0" cellspacing="0" cellpadding="0">
									<tr>
										<td align="right" class="a"><input name="radiobutton" type="radio" id="radiobutton_a" onClick="window.location='run_summary.shtml?enterprise_id=1122460&selectTarget=&channel='" value="radiobutton" checked /> <label for="radiobutton_a">按天统计</label> <input type="radio" name="radiobutton" id="radiobutton_b" value="radiobutton" onClick="changeType()" /> <label for="radiobutton_b">按时段统计</label></td>
										<td class="b">&nbsp;</td>
										<td class="c">开始时间： <input title="00:00:00" type="text" id="beginTime" size="15" readonly onClick="WdatePicker({isShowClear:false,dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'endTime\')||%y-%M-%d}'})" class="Wdate" value=""> <input id="beginTimeUtc" type="hidden" name="beginTime" /> &nbsp; 结束时间： <input title="23:59:59" type="text" id="endTime" size="15" readonly onClick="WdatePicker({isShowClear:false,dateFmt:'yyyy-MM-dd',maxDate:'%y-%M-{%d-1}'})" class="Wdate" value=""> <input id="endTimeUtc" type="hidden" name="endTime" /> <span class="btn-submit"><button type="submit" name="Submit">查 询</button></span>
										</td>
									</tr>
									<tr>
										<td colspan="2"></td>
										<td style="padding-top: 5px;"><span style="visibility: hidden;">开始时间： </span> <a class="blue" href="javascript:void(0)" onclick="queryByDate(2,true);return false;">昨天</a>&nbsp;|&nbsp; <a class="blue" href="javascript:void(0)" onclick="queryByDate(3,true,true);return false;">本周</a>&nbsp;<a class="blue" href="javascript:void(0)" onclick="queryByDate(4,true);return false;">上周</a>&nbsp;|&nbsp; <a class="blue" href="javascript:void(0)" onclick="queryByDate(5,true,true);return false;">本月</a>&nbsp;<a class="blue" href="javascript:void(0)" onclick="queryByDate(6,true);return false;">上月</a></td>
									</tr>
						...
						<div class="rr">
							<div class="yunxing">
								...
								<div class="pager">
									...
									</span> <em> <span class="btn-excel" onclick="toRunPrint();"><button type="button">打印</button></span> <span class="btn-excel" onclick="runSummary.doExport();"><button type="button">Excel</button></span>
									</em>
								</div>
	...
	<script type="text/javascript">
		...
		var runSummary = (function() {
			...
			var eid = '1122460', $listing = $('#listing'),
			...
			load = function(pageno) {
				...
				$.ajax({
					...
					success : function(json) {
						...
						if (print) {
							var content = $('<div></div>').append($('#listtable').clone()).html();
							content = '<div style="text-align:center;">运行总览(按天统计) ' + document.getElementById('beginTime').value + ' - ' + document.getElementById('endTime').value + '</div><br/>'
									+ content;
							Printer.view(content, false, './');
							print = false;
						}
			...
			render = function(json) {
				...
				html.push([ '<tr class="firstrow">', '    <td class="first"><span>序号</span></td>', '    <td><span>设备名称</span></td>', '    <td><span>里程(公里)</span></td>',
						'    <td><span>超速</span></td>', '    <td><span>停留(次)</span></td>', '</tr>' ].join('\n'));
				...
				if (len > 0) {
					...
					for (var i = 0, len = data.length; i < len; i++) {
						...
						var _outspeed = (outspeed == 0) ? outspeed : '<a href="javascript:changeRefer(\'overspeed_detail.shtml?enterprise_id=1122460&selectTarget=' + equipid + '&channel=\',\'' + name
								+ '\')">' + outspeed + '</a>';
						var _stop = (stop == 0) ? stop : '<a href="javascript:changeRefer(\'stay_detail.shtml?enterprise_id=1122460&selectTarget=' + equipid + '&channel=\',\'' + name + '\')">' + stop
								+ '</a>';
						html.push([ '<tr class="'+cls+'" data-user-name="'+ name +'" data-mileage="'+ milestat +'" data-out-speed="'+ outspeed +'" data-stay="'+ stop +'">',
								'<td>' + (i + 1) + '</td>',
								'<td><a href="javascript:changeRefer(\'run_index.shtml?enterprise_id=1122460&selectTarget=' + equipid + '&channel=\',\'' + name + '\')">' + name + '</a></td>',
								'<td>' + milestat + '</td>', '<td>' + _outspeed + '</td>', '<td>' + _stop + '</td>', '</tr>' ].join('\n'));
					}

					var total = json.summary;
					html.push([ '<tr class="listrow total">', '<td align="right"></td>', '<td></td>', '<td>' + total.totalmilestat + '</td>', '<td>' + total.totaloutspeed + '</td>',
							'<td>' + total.totalstop + '</td>', '</tr>' ].join('\n'));
				} else {
